...
inline std::vector<TeachingCase> teachingCases() {
  ...
  for (const bool sashimi : {false,true}) {
    ...
    const int finRow = sashimi ? 3 : 1;
    for (Cell c=0;c<81;++c) if (c/9==0 || c/9==finRow)
      fish.hintCandidates[c] = static_cast<CandidateMask>(fish.hintCandidates[c] & ~1U);
    if (sashimi) {
      for (const Cell c : {0,3,27,31,32}) fish.hintCandidates[c] |= 1;
    } else {
      for (const Cell c : {0,3,9,12,13,14}) fish.hintCandidates[c] |= 1;
    }
