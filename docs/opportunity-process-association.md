...
当前是诊断原型：离线工具与开发回顾详情共享算法，不改变现有回顾页面的去重统计、不建设成长档案、不向玩家发布掌握结论，也不更改 C++ 技巧或 humanCost。相同完整结果可有多种技巧解释；部分重叠的完整结果不强行合并。图中过程数不能作为使用次数。
...
此回归使用冻结日志盘面与规范操作，不读取或改写用户正在进行的游戏。开发回顾现可按需展示同一算法的复验结果，但不会回写历史标签；入口、状态和验收见[回顾原型](session-technique-review-prototype.md)。成长统计继续独立验收。
