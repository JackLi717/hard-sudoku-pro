...
- 数字键盘显示剩余数，固定工具栏覆盖撤销、擦除、快速候选、笔记和智能提示；两份草稿、
  当前来源、笔记模式和完整撤销历史都通过阶段 3 数据协议保存。
- 快速候选首次生成、同盘面恢复、盘面改变后的重新生成确认和额度扣减已接入；智能提示
...
3. 快速候选扣费、智能提示展示扣费、提示应用完成、首次奖励和最终钱包余额；
...
- iOS 26.5 的 iPhone 17 Pro 模拟器完成真实安装和启动。首页从 App 数据容器读取到
  `content-v4.sqlite` 的 10,000 道题，`user.sqlite` 自动迁移到 v2，初始额度为快速候选 3、
  智能提示 5。
