...
当前阶段签收方式以[自动化归因验收与持续体验](automated-behavior-acceptance.md)为准。`npm run behavior:acceptance` 将本工具纳入统一自动验收，报告写入独立运行目录；不再把玩家独立盲审作为工程签收前置条件。本文原有真人 TG-4 描述作为历史背景，不改变自动证据的边界。
