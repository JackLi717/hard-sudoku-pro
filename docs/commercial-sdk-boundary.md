...
日期：2026-09-10  
状态：7B-4 激励广告策略已收口；广告与购买的正式商店标识/商品配置、正式支持 URL 及两平台真机沙盒购买、广告与隐私验收仍待发布前完成。
...
公共契约只暴露标准激励广告及 `home_credit_store`、`credit_exhausted` 两个业务入口。普通插屏、激励插屏、横幅、原生、开屏和完成页广告不保留调用方法或广告位；未来如重新立项，必须重新完成产品、隐私与商店政策评审。广告位只使用稳定业务代码，不向 UI 暴露 SDK 的 ad unit 或原生对象。
...
- 免费用户请求首次生成快速候选数、确认重新生成候选数或使用智能提示而额度为 `0` 时，可进入解释页并逐次选择“观看广告，所选资源 `+1`”；不得先自动展示广告。快速候选数只有在有效奖励和额度发放均成功后才继续玩家原先请求的生成；取消或失败时保持候选数和显示状态不变。
...
Premium 不请求任何广告。开始游戏、继续游戏、游戏进行中、完成页及其他页面均不得自动展示广告；公共广告位中不存在 `game_completion`。
...
首期产品 ID 使用当前开发基线 `premium`，只代表一次性非消耗型 Premium；不预建订阅状态。当前本地 StoreKit 使用美国区 US$9.99 测试价，正式 iOS 与 Android 价格尚未确定；上线前须在各自商店后台配置。客户端继续展示商店返回的本地化价格。
...
- 未配置商品、测试环境不可用或平台不支持时，不产生假购买或假 Premium。发行版不展示假价格；开发版可显示明确标注且不可购买的 US$9.99 测试价格供页面评审。
...
- 两种资源不会串账，满库存不展示激励广告。
- 快速候选短按显示 / 隐藏已有候选数始终不扣额度、不触发广告。首次生成或确认后的重新生成在有额度时直接执行；额度为 `0` 时先展示明确的可拒绝领取说明，不自动播放，成功补额后才继续生成。
