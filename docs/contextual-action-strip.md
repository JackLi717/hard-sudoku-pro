...
Auto Complete 沿用现有资格判断：开启设置后，仅在困难、专家和极限难度、剩余少于 20 格、余下每步都是末格补全或唯一候选数且棋局没有错误时提供。点击 Strip 右侧操作才开始逐格填入；动画期间 Strip 隐藏，完成或资格消失后自动移除。原先位于棋盘上方的 `»` 入口不再显示。设置和界面使用“自动完成”语义，已保存的偏好字段与复盘动作标识保持不变。
