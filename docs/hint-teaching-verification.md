...
| multiColoring | 通过 | 当前明确覆盖 HoDoKu Multi Colors Type 1：两分量互斥色导致至少一个反色为真；目标见两反色，不宣称覆盖其他类型 |
| remotePair | 通过 | 双值格的二分连通结构；不用排序坐标冒充线性链 |
| emptyRectangle | 通过 | 保留既有专用宫内交叉与强对证明；不属于唯一性 |
| uniqueRectangleType4 | 通过 | 两个精确双值角、两个含额外候选角；后两角在行、列或宫内形成一个矩形数字的强链，并从两角删除另一个矩形数字 |
| hiddenRectangle | 通过 | 一个精确双值角及其对角目标；同一个矩形数字分别在目标行、目标列形成经过目标格的两条强链，只从目标格删除另一个数字 |
...
| aic | 通过 | 当前覆盖 HoDoKu 不连续闭环结论：回到起始候选并得到相反真假状态，支持假真导假删除、假假导真填数；不宣称覆盖开放式 Type 1 / Type 2 |
| groupedAic | 通过 | 单数字候选 OR 组，真实组强弱边，最多 7 条边；双端组排除 |
| complexColoring | 通过 | 产品自定义分类名，HoDoKu 无同名独立技巧；按一般染色/链规则，至少三个真实分量传播并回到起始分量反色形成矛盾 |
...
- `__tests__/hint-teaching.test.ts` 验证 40 项与分支变体、完整集合、区域角色、真实候选、依赖破坏回退、四语、原始结果不可变与复盘一致性；`sudoku-board-hints.test.tsx` 检查实际颜色/组/假设组件。

## 本轮实际验证结果（2026-09-05）

- `npm run format:check`、`npm run lint`、`npm run typecheck`：通过，最终 lint 无警告。
- `npm run schema:check`：通过。
- `npm run hint:core:check`：严格 C++20 编译、单元测试、回放，以及全部 40 技巧的正例/负例/安全结果测试通过。
...
- Hint Lab：40 个目录图例及补充变体均由原生导出。新生成内容通过候选合法性、原子结果与参考解检查；导出成功后原子替换开发 JSON。
