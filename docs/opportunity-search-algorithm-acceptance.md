...
完整执行目标 fixture 后，旧的严格 sequence matcher 只有10项产生单一技巧候选；13项因多个技巧解释相同完整动作而进入 `ambiguous`，16项因更长的重叠 identity 仍可能成立而保持 `matching`。16项未决案例的存活 identity 全部跨技巧，没有“仅同一技巧但 outcome 长度不同”的安全提前候选；因此当前证据否决放宽该状态机的关闭条件。后两类共29项在严格状态机中保守放弃，不把“已做完目标 fixture”错误等同于“已证明玩家使用目标技巧”。它们不是产品层最终的 `candidateTechniques`；最终候选与最低成本默认解释统一由《玩家技巧归因协议》和第10节结果解释器决定。完整冲突技巧集合与 identity 完成数量写入生成报告，作为下一轮人工 effect/证明真值目录。
