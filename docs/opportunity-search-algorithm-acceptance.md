...
完整执行目标 fixture 后只有10项产生技巧候选；13项因多个技巧解释相同完整动作而进入 `ambiguous`，16项因更长的重叠 identity 仍可能成立而保持 `matching`。后两类共29项全部保守放弃，不把“已做完目标 fixture”错误等同于“已证明玩家使用目标技巧”。这组正例回放证明状态机的不变量在39项真实 outcome 形状上成立，但不能测量真实玩家路径中的误判率，也不能决定是否应增加显式序列关闭信号。
