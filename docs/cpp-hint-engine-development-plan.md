...
核心库不依赖 React Native、JSI、SQLite、界面、本地化、题目答案或 HoDoKu。技巧只返回稳定代码、格子、区域、前提、排除和填数；四语言文字由 App 根据模板渲染。答案只允许在库外的测试安全校验中使用。
